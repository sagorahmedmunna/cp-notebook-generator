...
\begin{enumerate}
	...
	\item $1^3 + 2^3 + \dots + n^3 = \left(\frac{n(n+1)}{2}\right)^2$

	\item \textbf{Arithmetic Progression (AP):} $a + (a+d) + \dots + (a+(n-1)d) = \frac{n}{2}\left(2a + (n-1)d\right)$

	\item \textbf{Geometric Progression (GP):} $1 + r + r^2 + \dots + r^n = \dfrac{r^{n+1} - 1}{r - 1}$, for $r \ne 1$

	\item \textbf{Infinite GP (} $|r| < 1$ \textbf{):} $1 + r + r^2 + \dots = \dfrac{1}{1-r}$

	\item \textbf{Quadratic Formula:} $ax^2 + bx + c = 0 \Rightarrow x = \dfrac{-b \pm \sqrt{b^2 - 4ac}}{2a}$
